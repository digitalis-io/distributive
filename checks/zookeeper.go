...
func (chk ZooKeeperServerStats) Status() (int, string, error) {
	...
	for i, ok := range oks {
        ...
        if ok.MaxLatency > chk.maxLatency {
            failed = append(failed, fmt.Sprintf("%s: min latency too big: %v", chk.servers[i], ok.MaxLatency)) 
        }
        if ok.AvgLatency > chk.avgLatency {
            failed = append(failed, fmt.Sprintf("%s: min latency too big: %v", chk.servers[i], ok.AvgLatency)) 
        }
    }
	if len(failed) == 0 {
		return errutil.Success()
